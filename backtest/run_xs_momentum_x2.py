"""
run_xs_momentum_x2.py — runda X2: momentum przekrojowe (rodzina B1) jak X1, ale uniwersum
point-in-time TOP-50 po obrocie (nogi po 10 = kwintyle), od 2021-05-01 (pierwszy miesiąc
z ≥ 50 kandydatami). Reszta identyczna z X1: sygnał 28 dni, trzymanie 7 dni, koszty 0,07 % × obrót,
funding per symbol. Pre-rejestracja (PRZED tym skryptem): runs/2026-09-23_x2-momentum-top50/README.md.

Uruchomienie:  py -m backtest.run_xs_momentum_x2 [universe_dir]   (Windows: PYTHONUTF8=1)
               py -m backtest.run_xs_momentum_x2 --moc [n_sim]      # rachunek mocy (losowe rankingi)

Skrypt X1 jest ZAMROŻONY (zasada 13) — ten jest jego kopią z trzema stałymi (TOP_N, LEG_SIZE,
FIRST_MONTH); maszyneria w backtest/xs_momentum.py bez zmian. Neutralny reporter.
"""
...
TOP_N = 50  # X2: szerokość zamiast wariantu sygnału
LEG_SIZE = 10  # kwintyle z 50
FIRST_MONTH = "2021-05-01"  # pierwszy miesiąc z ≥ 50 kandydatami (luty–kwiecień 2021: 41–46)
...
def _load(universe_dir: str):
    ...
    members = monthly_members(volume, month_starts, top_n=TOP_N)
...
def _print_header(fee: float, close, members, dates) -> None:
    ...
    print(
        ...
        f"{len(members)} koszyków miesięcznych TOP-{TOP_N} po obrocie ({n_syms} różnych symboli), "
...
def run_power(universe_dir: str, n_sim: int) -> None:
    ...
    print(SEP)
    print(
        f"X2 — RACHUNEK MOCY Z SYMULACJI: {n_sim} losowych rankingów na realnym panelu TOP-{TOP_N} (bez patrzenia na sygnał)"
    )
    print(SEP)
    _print_header(fee, close, members, dates)
    sims = simulate_null_means(
        close, funding, members, dates, fee, n_sim=n_sim, seed=0, leg_size=LEG_SIZE
    )
    ...
    print(
        f"  EMPIRYCZNE se średniej dziennej pod H0 = {100 * se_emp:.4f}%/dzień → half-width 95% = "
        f"{100 * Z95 * se_emp * DAYS_PER_YEAR:.1f}%/rok; z korektą ×1,3 (wniosek 65): "
        f"{100 * 1.3 * Z95 * se_emp * DAYS_PER_YEAR:.1f}%/rok"
    )
...
def main(argv: list[str]) -> None:
    ...
    print(SEP)
    print(
        f"X2 — MOMENTUM PRZEKROJOWE (B1) NA TOP-{TOP_N}: long top-{LEG_SIZE} / short bottom-{LEG_SIZE} po zwrocie 28 dni, trzymanie 7 dni; konfiguracja ZAMROŻONA"
    )
    print(SEP)
    _print_header(fee, close, members, dates)
    ...
    print(
        ...
        f"dni z nogą < {LEG_SIZE} aktywnych: {int(((out['n_long'] < LEG_SIZE) | (out['n_short'] < LEG_SIZE)).sum())}"
    )
