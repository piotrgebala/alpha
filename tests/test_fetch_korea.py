...
def _fake_api(monkeypatch, ignore_to: bool = False) -> list:
    """Udawane API Upbit: ≤ 200 świec PRZED `to`, malejąco; `ignore_to` — zawsze najnowsze."""
    calls = []

    def fake_http_get(url: str) -> bytes:
        to = pd.Timestamp(url.split("to=")[1].replace("%3A", ":"))
        calls.append(to)
        cut = DAYS[-1] if ignore_to else to
        before = [d for d in DAYS if d < cut][-fk.UPBIT_MAX_CANDLES :]
...
def test_fetch_upbit_daily_stops_early_and_filters_before_start(tmp_path, monkeypatch):
    calls = _fake_api(monkeypatch)
    now = DAYS[-1] + pd.Timedelta(hours=5)
    path = fk.fetch_upbit_daily("KRW-BTC", str(DAYS[250].date()), tmp_path, now=now)
    df = pd.read_parquet(path)
    assert len(calls) == 1  # pierwsza paczka (dni 249–448) już sięga przed start
    assert df["open_time"].min() == DAYS[250]  # dzień 249 odfiltrowany
    assert df["open_time"].tolist() == list(DAYS[250:-1])


def test_fetch_upbit_daily_fails_loud_when_cursor_does_not_move(tmp_path, monkeypatch):
    _fake_api(monkeypatch, ignore_to=True)
    now = DAYS[-1] + pd.Timedelta(days=3)  # `to` po ostatniej świecy → druga paczka identyczna
    with pytest.raises(ValueError, match="kursor nie cofa się"):
        fk.fetch_upbit_daily("KRW-BTC", "2019-01-01", tmp_path, now=now)
